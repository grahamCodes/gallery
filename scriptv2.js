...
addMore.setAttribute("onclick", "picrun()");
const h2 = document.createElement("h2");
h2.textContent = "Show More";
addMore.appendChild(h2);
const p = document.createElement("h2");
p.textContent = "+";
addMore.appendChild(p);
...
function picrun() {
  for (i = j; i < j + 3; i++) {
    ...
    img.setAttribute("alt", `A picture of a ${picture.title}`);
    div.appendChild(img);
    const h3 = document.createElement("h3");
    h3.textContent = picture.title;
    div.appendChild(h3);
    const h4 = document.createElement("h4");
    h4.textContent = `${picture.creator} ~ ${picture.price}`;
    div.appendChild(h4);
  }
  main.appendChild(addMore);
  j = j + 3;
  if (j == 15) {
    addMore.remove();
  } else {
